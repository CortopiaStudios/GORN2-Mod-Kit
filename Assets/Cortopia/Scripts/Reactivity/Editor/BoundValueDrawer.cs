...
{
    [CustomPropertyDrawer(typeof(BoundValue<>))]
    public class BoundValueDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if ((referenceChanged || !serializedReference.hasMultipleDifferentValues) && pickedObj)
            {
                ...
                Type bindableType = typeof(IBindableReactive<>).MakeGenericType(genericArguments);
                ...
                var bindings = monoBehaviours.SelectMany(obj =>
                    {
                        ...
                        return objType.GetProperties()
                            .Select(p =>
                            {
                                ...
                                bool isValid = isReactive || genericArguments[0].IsAssignableFrom(p.PropertyType);
                                if (!isValid)
                                {
                                    return null;
                                }

                                object bindableReactive;
                                try
                                {
                                    bindableReactive = p.GetValue(obj);
                                }
                                catch (Exception)
                                {
                                    return null;
                                ...
                                bool isReadOnly = isReactive ? ((IBindableReactive) bindableReactive).IsReadOnly : !p.CanWrite || !p.SetMethod.IsPublic;
                                ...
                                return new Bindable.Property(obj, p.Name, name, isReactive, isReadOnly);
                            })
            ...
            if (EditorApplication.isPlaying)
            {
                var boundValues = ReactiveDrawerHelpers.GetObjects<IBoundValue>(property).ToArray();
                bool readOnly = boundValues.Any(x => x.IsReadOnly);
                if (!readOnly)
                {
                    EditorGUI.EndDisabledGroup();
                }
                ...
                foreach (IBoundValue boundValue in boundValues)
                {
        ...
        private static string BindableName(Bindable bindable)
        {
            ...
            {
                Bindable.Property p => $"{(p.IsReactive ? "" : "[non-reactive] ")}{p.Name}{(p.IsReadOnly ? " [read only]" : "")}",
        ...
        private abstract record Bindable
        {
            ...
            public record Property(Object Object, string PropertyName, string Name, bool IsReactive, bool IsReadOnly) : Bindable
            {
                public Object Object { get; } = Object;
